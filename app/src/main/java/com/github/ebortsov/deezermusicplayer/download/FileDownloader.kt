 ...
 *
 * This class uses [OkHttpClient] so it should be used as a singleton
 */
    ...
    suspend fun download(destination: File, url: String): Boolean {
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                // Try so save the destination in temporary (.tmp) file
                // If the saveResponseBody returns true, then rename the .tmp file to destination filename
